...
const Edit = () => {
    ...
    useEffect(() => {
        axios.get(`/password/${id}`, config).then((response) => {
            var info = response.data.password;
    ...
    const submitHandler = () => {
        ...
        axios.post(`/password/update/${id}`, { website, email, password }, config).then((response) => {
            navigate("/");
        }).catch((error) => {
            ...
            <div className="w-full flex justify-center">
                <div className="w-[350px] sm:w-[500px] flex flex-col justify-center py-8 px-10">
                    <div className="flex justify-center">
                        <span className="text-xl font-bold">Edit a Password</span>
                    </div>
